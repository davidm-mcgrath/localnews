...
#relabel local stories in natl dataset
...
```
##Further breaking down news topic
```{r,results='asis'}
...
econg=ggplot(data = econ_gr,aes(quarter)) +
  ...
  scale_x_continuous(breaks = seq(1,24,4),
                     ...
                     limits = c(1,24))
econg
cor(econ_gr$prop_positive,econ_gr$gdp)
```
...
```{r, results='asis'}
sample_headlines=filter(nptext, topic%in%c(1,4,5,6) & score>0.99) %>%     select(headline, headline_plus, label)
sh2=sample_headlines[sample(1:nrow(sample_headlines),20),]
...
circ_in_out
```
##NP Circulation and Jobs Over Time
```{r,results='asis'}
library(haven)
setwd("C:/Users/dm4947/OneDrive/Documents/NYU/Dissertation/Data/NewspaperData")
